...
object Lab6 extends jsy.util.JsyApplication {
  ...
  object REParser extends Parsers {
    ...
    def union(next: Input): ParseResult[RegExpr] = intersect(next) match {
      case Success(r, next) => {
        def unions(acc: RegExpr, next: Input): ParseResult[RegExpr] =
          if (next.atEnd) Success(acc, next) //if empty string move on in grammar
          else (next.first, next.rest) match { //else look at next avail char
            case ('|', next) => intersect(next) match { //if valid '|' for union
              case Success(r, next) => unions(RUnion(acc, r), next) // recursive call for '|' intersect unions
              case _ => Failure("expected intersect", next)
            }
            case _ => Success(acc, next) // epsilon; move on in grammar
          }
        unions(r, next)
      }
      case _ => Failure("expected intersect", next)
      ...
      case Success(r, next) => {
        def intersects(acc: RegExpr, next: Input): ParseResult[RegExpr] =
          if (next.atEnd) Success(acc, next) //if empty string move on in grammar
            ...
            case ('&', next) => concat(next) match { //if valid '&' for intersect
              case Success(r, next) => intersects(RIntersect(acc, r), next) // recursive call for '&' concat intersects
    ...
    def not(next: Input): ParseResult[RegExpr] = (next.first, next.rest) match {
      case ('~', rest) => not(rest) match {
          //check to see if there is a neg ('~') in the remaining.
        case Success(r, next) => Success(RNeg(r), next) // if yes, successful.
        case fail => fail
      }
        // if no neg, then we check star
      case _ => star(next) match {
        case Success(r, next) => Success(r, next) // if yes, then successful
        case fail => fail
      }
    }
    //  *   star ::= atom stars
    ...
    def star(next: Input): ParseResult[RegExpr] = atom(next) match {
        //retrieve atom of next
      case Success(r, next) => {
        def stars(acc: RegExpr, next: Input): ParseResult[RegExpr] =
          // stars defined
          if (next.atEnd) Success(acc, next)
            //if next is the end of regex, return success
          else (next.first, next.rest) match { //consume one character
              //otherwise check for symbols
            case ('*', next) => stars(RStar(acc),next) // zero or more
            case ('+', next) => stars(RPlus(acc), next) // one or more
            case ('?', next) => stars(ROption(acc), next) // zero or one
    ...
    def atom(next: Input): ParseResult[RegExpr] =
    {
      ...
      else (next.first, next.rest) match { //check cases from handout, match our symbols first
        case ('!', next) => Success(RNoString, next)
        case ('#', next) => Success(REmptyString, next)
        case ('.', next) => Success(RAnyChar, next)
        case ('(', next) => re(next) match { //if '(' we have found another regex call re on it to parse that.
          case Success(r, next) => (next.first, next.rest) match { // consumes the regex inside the parentheses
            case (')', next) => Success(r, next) // closed, call success on regex and next node.
            case _ => Failure("unmatched )", next)
          }
          case _ => Failure("expected match", next)
        }
        case (c, next) if !delimiters.contains(c) => Success(RSingle(c), next) // from class
        case _ => Failure("expected atom", next)
      }
    }
    ...
    def parse(next: Input): RegExpr = re(next) match {
      case Success(r, next) if (next.atEnd) => r // all chars consumed in our input.
      case Success(_, next) => throw new SyntaxError("remaining input", next.pos) //grammar didn't get it all
      case Failure(msg, next) => throw new SyntaxError(msg, next.pos) // fail
    }
  ...
  def retest(re: RegExpr, s: String): Boolean = { //examine regex and string.
    // test: see if a prefix of chars matches the regular expression. if yes, the sc(success continuation) called with
    // remaining chars to be matched.
    // if any early failure then return false.
    def test(re: RegExpr, chars: List[Char], sc: List[Char] => Boolean): Boolean = (re, chars) match {
      ...
      case (RSingle(c1), c2 :: t) => c1 == c2 && sc(t)  //our prefix and head of list better match, then list should be empty
      case (RConcat(re1, re2), _) => test(re1, chars, {chars1 => test(re2, chars1, sc)}) // test the first re, the continuation tests re2 (anonymous func).
      case (RUnion(re1, re2), _) => test(re1, chars, sc) || test(re2, chars, sc) // can do either one, "|".
      case (RStar(re1), _) => sc(chars) || test(re1, chars, { chars1 => if (chars1.size < chars.size) test(RStar(re1), chars1, sc) else false })
      // check if list is empty. if false, then check similar to concat except we are looking for concatenations of re1.
      // both return false in order for match failure.
      // zero or more concatenations of re1.
      /* Extended Operators */
